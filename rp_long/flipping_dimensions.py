...
import time
import copy
from skmultiflow.data import ConceptDriftStream, STAGGERGenerator, SEAGenerator, FileStream, LEDGeneratorDrift, MIXEDGenerator, SineGenerator, LEDGenerator
from skmultiflow.lazy import SAMKNN
from skmultiflow.trees import HAT
from skmultiflow.meta import AdaptiveRandomForest, OzaBaggingAdwin
...
def low_dim_test(stream, clf, n_samples, flip=True):
    ...
    ax = plt.gca()
    ax.set_ylim([0, 1.05])
    plt.savefig('res_{}_{}.png'.format(stream.name, clf.name))
    plt.clf() # clear plot
...
#        arslvq = RRSLVQ(confidence=.0005)
        arslvq = OzaBaggingAdwin()
        arslvq.name = 'RRSLVQ'
        low_dim_test(copy.copy(stream), copy.copy(arslvq), N_SAMPLES)
        
        arslvq = OzaBaggingAdwin()
        arslvq.name = 'RRSLVQ'
        low_dim_test(copy.copy(stream), copy.copy(arslvq), N_SAMPLES, flip=False)
